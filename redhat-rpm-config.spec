...
Patch16: redhat-rpm-config-9.0.3-RHEL6-ksyms-updates3.patch
Patch17: redhat-rpm-config-9.0.3-RHEL6-kmodtool-fix-nobuildreqs.patch
Patch18: redhat-rpm-config-9.0.3-RHEL6-macros-kernel_module_package_moddir.patch
Patch19: redhat-rpm-config-9.0.3-RHEL6.2-ksyms-fixes.patch
Patch20: redhat-rpm-config-9.0.3-configure-path.patch
Patch21: redhat-rpm-config-9.0.3-RHEL6.3-fix-ksym-generation.patch
Patch22: redhat-rpm-config-9.0.3-RHEL6.3-fix-kmod-package.patch
Patch23: redhat-rpm-config-9.0.3-RHEL6.3-add-kabi-check.patch
Patch24: redhat-rpm-config-9.0.3-RHEL6.3-fix-find-provides-modalias-permissions.patch
Patch25: redhat-rpm-config-9.0.3-RHEL6-kmodtool-fix-variant.patch
Patch26: redhat-rpm-config-9.0.3-RHEL6-smp-ncpus-max.patch
Patch27: redhat-rpm-config-9.0.3-RHEL6-brp-java-repack-jars-fix-permissions.patch
Patch28: redhat-rpm-config-9.0.3-no-macrofiles.patch
Patch29: redhat-rpm-config-9.0.3-kabi.patch
Patch30: redhat-rpm-config-9.0.3-shebang.patch
Patch31: redhat-rpm-config-9.0.3-latest-kernel.patch
Patch32: redhat-rpm-config-9.0.3-permit-different-python.patch
Patch33: redhat-rpm-config-9.0.3-ksyms.patch
Patch34: redhat-rpm-config-9.0.3-repack-jars.patch
Patch35: redhat-rpm-config-9.0.3-file-output.patch
Patch36: redhat-rpm-config-9.0.3-file-output2.patch
Patch100: redhat-rpm-config-recognize-eurolinux.patch

BuildArch: noarch
Requires: mktemp
Provides: system-rpm-config
...
%changelog
* Tue Aug  2 2016 Aleksander Baranowski <aleksander.baranowski@euro-linux.com> - 9.0.3.51
- Patches from upstream including:
    + fixed another problems with changes in file output (#1321988)
    + fixed problem with changes in file output (#1321988)
    + Restore UID/ GID on jar files (#1270052)
    + Replace '\t' with $'\t'. '\t' is only '\t' but $'\t' is a tabulator (#1303249)
    + Fix wrong generating ksym(...) requires instead of kernel(...) (#1303249)
    + Remove aarch64 part from regex for the latest kernel. aarch64 is not
      supported in rhel+6 (#976462)
    + Fix latest_kernel macro (#976462)
    + Permit using different python version for brp-python-bytecompile (#1252628)

* Mon Sep 14 2015 Aleksander Baranowski <aleksander.baranowski@euro-linux.com> - 9.0.3-44.1
- Release up	
- Added redhat-rpm-config-recognize-eurolinux.patch
- EuroLinux is an Enterprise Linux, modify the script to know this

* Mon Mar 09 2015 Florian Festi <ffesti@redhat.com> - 9.0.3-44
- Add Provides: system-rpm-config
- Resolves: #1199983
* Tue Mar 03 2015 Florian Festi <ffesti@redhat.com> - 9.0.3-43
- Adjust location of kabi-whitelists
- Resolves: #842761
- Be stricter with she bang lines in find-requires
- Resolves: #1179521

* Tue Oct 30 2012 Panu Matilainen <pmatilai@redhat.com> - 9.0.3-42
- Remove unused macrofiles directive from rpmrc
- Resolves: #869062

* Mon Oct 15 2012 Panu Matilainen <pmatilai@redhat.com> - 9.0.3-41
- Zero or unset _smp_ncpus_max macro means no CPU limit
- Related: #669638

* Sun Oct 14 2012 Florian Festi <ffesti@redhat.com> - 9.0.3-40
- Fix verrel in kmodtool when used with variants
- Resolves: #795577
- Add %%_smp_ncpus_max macro to allow adjusting the number of processors used
- Resolves: #669638
- Fix brp-java-repack-jars for jar files that contain non-standard permissions
- Resolves: #822073

* Tue Mar 06 2012 Jon Masters <jcm@redhat.com> - 9.0.3-39
- Fix permissions on /usr/lib/rpm/redhat/find-provides.d/modalias.prov
- Resolves: #713638

* Mon Mar 05 2012 Jon Masters <jcm@redhat.com> - 9.0.3-38
- Add an informational check of kABI during kernel module build.
- Resolves: #713992

* Mon Mar 05 2012 Jon Masters <jcm@redhat.com> - 9.0.3-37
- Fix a problem with "-v" and "-r" kernel_module_package options.
- Resolves: #680029

* Mon Mar 05 2012 Jon Masters <jcm@redhat.com> - 9.0.3-36
- Fix ksym/kernel generation (correct problem with previous fix).
- Resolves: #767738

* Fri Mar 02 2012 Panu Matilainen <pmatilai@redhat.com> - 9.0.3-35
- Permit overriding path to configurescript (#652084)

* Mon Aug 22 2011 Jon Masters <jcm@redhat.com> - 9.0.3-34
- Fix ksym/kernel dependency generation in driver updates.
- Resolves: #642768, #681884.

* Fri Feb 04 2011 Jon Masters <jcm@redhat.com> - 9.0.3-33
- Implement the kernel_module_package_moddir macro.
- Resolves: #658280.

* Thu Feb 03 2011 Jon Masters <jcm@redhat.com> - 9.0.3-32
- Fix overrides of BuildRequires and default kernel variant requirement.
- Resolves: #642768.

* Wed Feb 02 2011 Jon Masters <jcm@redhat.com> - 9.0.3-30
- Bump to release "30" to handle internal build system tag conflict.
- Resolves: #642768.

* Mon Jan 24 2011 Jon Masters <jcm@redhat.com> - 9.0.3-27
- Update the find-provides.ksyms script to add '0x' prefix to checksums
- Resolves: #642768.
